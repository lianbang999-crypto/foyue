...
// Fix audio fallback losing time
...
s = s.replace(/setTimeout\(\(\) => \{\s*if \(dom\.audio\.src === src\) \{\s*setBuffering\(true\);\s*dom\.auds = s.replace(/setTimeout\(\(\) => \{
\s *if ifs = s.replace(/setTimeout\(\(\) => \{\s*if \(dom\.audio\.src === srt s = s.replace(/setTimeout\(\(\) = s = s.replace(/setTntListener('loadedmes = s.replace(/setTimeout\(\(\) => \{
\s *if \(dom\.audio\.src === src       dom.audio.load(); `);
