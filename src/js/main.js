...
function openAiChat() {
  ...
  getAiChatModule().then(m => {
    m.openAiChat();
  }).catch(() => {
    showToast('AI 功能暂时无法加载，请刷新页面重试');
  }).finally(() => {
...
async function loadData() {
  ...
  try {
    ...
    document.getElementById('retryLoadBtn').addEventListener('click', () => {
      loadAttempts = 0;
      dom.loader.innerHTML = '<img src="/icons/loading-logo.png" style="width:120px;height:auto;opacity:.4;animation:breathe 2.5s ease-in-out infinite" alt=""><div class="loader-text">Loading...</div>';
      loadData();
    });
  }
}
