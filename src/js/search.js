...
export function doSearch(q, showEpisodes, renderCategory, renderHomePage) {
  ...
  if (!totalCount) {
    ...
    wrap.innerHTML = `<div class="search-label">${t('search_results')} (${totalCount})</div>`;
    ...
    if (epResults.length) {
      ...
      epResults.slice(0, 50).forEach(r => {
        const li = document.createElement('li');
        li.className = 'ep-item';
        li.innerHTML = `<span class="ep-num">${r.ep.id || r.idx + 1}</span><span class="ep-title">${highlight(r.ep.title || r.ep.fileName, q)} <small style="color:var(--text-muted)">\u00B7 ${r.series.title}</small></span>`;
