...
function buildCounterDesc(counterData) {
  let desc = t('my_counter_desc');
  if (counterData?.practices) {
    ...
    if (ps && ps.total > 0) {
      const displayName = practice === '__custom__'
        ...
        : escapeHtml(practice);
      desc = t('my_counter_total_desc').replace('{n}', ps.total) + ' · ' + displayName;
    }
  } else if (counterData?.total > 0) {
    desc = t('my_counter_total_desc').replace('{n}', counterData.total)
      + (counterData.practice ? ' · ' + escapeHtml(counterData.practice) : '');
  }
  return desc;
}
...
export function renderMyPage() {
  ...
  const counterDesc = buildCounterDesc(storeGet('counter') || {});
  const installHTML = buildInstallSection();
    ...
    <div class="my-section">
      ...
      <div class="my-list">
        ...
        <div class="my-item" id="myThemeItem">
          <svg class="my-item-icon" viewBox="0 0 24 24">${getTheme() === 'dark'
