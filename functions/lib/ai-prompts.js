...
const FOLLOWUP_BLOCK_RE = /\[FOLLOWUP\]([\s\S]*?)\[\/FOLLOWUP\]/i;
const QUESTION_STOP_WORDS_RE = /什么|怎么|怎样|如何|为什么|哪些|哪个|可以|能够|应该|是不是|有没有|到底|究竟|请问|一下|的|了|吗|呢|吧|啊|在|是|有|和|与|或|也|都|就|把|被|对|又|要|让|给|从|用|以|而|但|却|不|很|最|更|还|这|那|它|你|我|他|她|们|个|着/g;
...
function deriveQuestionTopic(question) {
    const cleaned = String(question || '')
        .replace(QUESTION_STOP_WORDS_RE, ' ')
        .replace(/\s+/g, ' ')
        .trim();
...
export function normalizeAiAnswerContract(rawText, question, options = {}) {
    ...
    const noResult = forceNoResult || /未找到相关内容/.test(body);
    ...
    for (const item of parsedFollowUps) {
        ...
        if (uniqueFollowUps.length >= 2) break;
    }

    const followUps = uniqueFollowUps.length >= 2
        ? uniqueFollowUps
